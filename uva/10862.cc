...
int main(void)
{
  ...
  //
  // Note that all minimum wire connections have n edges
  //
  // We use the following recurrence:
  //
  // f(n) = sum(k * f(n-k), k = 1..n)
  //
  // Each term in sum: the first k houses are all connected to each other
  //                   and one of these k houses is connected to the station.
  //                   The rest are connected "recursively".
  //
