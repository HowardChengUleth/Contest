...
#define SRC 0
#define SINK 1
#define COMPUTER(x) ((x)+2)
#define APP(x) ((x)-'A'+12)
#define INVAPP(x)  ((char)((x)-12+'A'))
...
bool solve()
{
  ...
  while (getline(cin, s) && s != "") {
    ...
    int num = s[1] - '0';
    total += num;
    G.add_edge(APP(job), SINK, num);
    
    for (int i = 3; i < s.length()-1; i++) {
      G.add_edge(COMPUTER(s[i]-'0'), APP(job), 1);
    }
  }
  ...
  if (flow != total) {
    ...
    for (int i = 0; i < 10; i++) {
      bool assigned = false;
      for (EdgeIter it = G.nbr[COMPUTER(i)].begin(); 
	   it != G.nbr[COMPUTER(i)].end(); ++it) {
