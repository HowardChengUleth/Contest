...
void compute(vector< vector<BigInteger> > &table, int i, int j, 
	     const string &X, const string &Z)
{
  if (!(table[i][j] == -1)) {
    return;
  }
  ...
  if (X[i-1] != Z[j-1]) {
    compute(table, i-1, j, X, Z);
    table[i][j] = table[i-1][j];
    return;
  }
  
  compute(table, i-1, j, X, Z);
  compute(table, i-1, j-1, X, Z);
...
void solve(void)
{
  ...
  /*
   * table[i][j] = number of subsequences of Z[1..j] in X[1..i]
   *
   * table[0][j] = 0
   * table[i][0] = 1
   * table[i][j] = 0    if j > i
   * table[i][j] = table[i-1][j] if X[i] != Z[j]
   * table[i][j] = table[i-1][j] + table[i-1][j-1] otherwise
   *
   */

  vector< vector<BigInteger> > table;

  cin >> X >> Z;

  table.resize(X.length()+1);
  for (unsigned int i = 0; i <= X.length(); i++) {
    table[i].resize(Z.length()+1);
    for (unsigned int j = 0; j <= Z.length(); j++) {
      table[i][j] = -1;
    }
  }

  compute(table, X.length(), Z.length(), X, Z);
  /*
  for (unsigned int i = 0; i <= X.length(); i++) {
    for (unsigned int j = 0; j <= Z.length(); j++) {
      cout << table[i][j] << ' ';
  ...
  cout << "====================" << endl;
  */
  cout << table[X.length()][Z.length()] << endl;
}
