...
bool can_do(int cutoff, int k)
{
  for (int i = 0; i < num_blue; i++) {
    for (int j = 0; j < num_red; j++) {
      bipgraph[i][j] = (sqdist(blue[i], red[j]) <= cutoff*cutoff);
    }
  }
  match(num_blue, num_red);
  int count = 0;
  for (int i = 0; i < num_blue; i++) {
    if (matching[i] != -1) {
      count++;
...
void solve_case()
{
  ...
  for (int i = 0; i < P; i++) {
    ...
    cin >> x >> y >> c;
    if (c == "blue") {
      blue[num_blue++] = make_pair(x,y);
    } else {
      red[num_red++] = make_pair(x,y);
    }
  }
  ...
  // invariant: cannot do "lo", but can do "hi"
  int hi = 3000;
  int lo = 0;
  while (hi - lo > 1) {
    int mid = (lo+hi)/2;
