...
int main(void)
{
  ...
  while (cin >> n) {
    if (n == 0) {
      ...
      /* there are n*(n-1)/2 pairs of circles, so n*(n-1) intersections.
	 Look at the planar graph where vertices are the intersections
	 and edges (possibly multiple) are the arcs connecting vertices.
	 Every vertex has degree 4, so the total number of edges is
	 2*n*(n-1).  Finally, apply Euler's formula f = e - v + 2 to
	 get the answer */
      cout << n*(n-1)+2 << endl;
    }
  }
