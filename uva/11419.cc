// idea: form a bipartite graph with rows on one side and columns on the other
//
// add an edge from row i to col j if there is an enemy at (i,j)
//
// we are looking for a min vertex cover, which can be obtained from the
// max matching.
...
int main(void)
{
  ...
  while (cin >> R >> C >> N && (R || C || N)) {
    for (int i = 0; i < R; i++) {
      fill(bipgraph[i], bipgraph[i]+C, 0);
    }
    for (int i = 0; i < N; i++) {
      int r, c;
      cin >> r >> c;
    ...
    // form T which are all vertices reachable from unmatched vertices
    // from the left by alternating paths
    bool leftT[1000], rightT[1000];
    int left_queue[1000], right_queue[1000];
    int left_size = 0, right_size = 0;
    int rev_matching[1000];
    
    fill(leftT, leftT + R, false);
    fill(rightT, rightT + C, false);
    fill(rev_matching, rev_matching + C, -1);
    
    int cover_size = 0;
    for (int i = 0; i < R; i++) {
      if (matching[i] != -1) {
	rev_matching[matching[i]] = i;
	cover_size++;
      } else {
	leftT[i] = true;
	left_queue[left_size++] = i;
      }
    }

    // keep exploring
    while (left_size > 0) {
      // let's take each element on left and go to the right
      right_size = 0;
      for (int i = 0; i < left_size; i++) {
	int u = left_queue[i];
	for (int v = 0; v < C; v++) {
	  if (bipgraph[u][v] && !rightT[v] && matching[u] != v) {
	    rightT[v] = true;
	    right_queue[right_size++] = v;
	  }
	}
      }

      // now take each element on right and go back to the left
      left_size = 0;
      for (int i = 0; i < right_size; i++) {
	int v = right_queue[i];
	int u = rev_matching[v];
	if (u == -1) continue;
	leftT[u] = true;
	left_queue[left_size++] = u;
      }
    }
    
    cout << cover_size;

    // min cover is (L - T) U (R \cap T)
    for (int i = 0; i < R; i++) {
      if (!leftT[i]) {
	cout << " r" << i+1;
      }
