...
int main(void)
{
  ...
  convert_from_int(f[0], 1);
  convert_from_int(f[1], 2);
  for (i = 2; i < 120; i++) {
    add(f[i-2], f[i-1], f[i]);
  }
  ...
  while (scanf("%s %s", A, B) == 2) {
    ...
    for (i = strlen(A)-1; i >= 0; i--) {
      if (A[i] != '0') {
	add(sum, f[strlen(A)-1-i], sum);
      }
    }
    for (i = strlen(B)-1; i >= 0; i--) {
      if (B[i] != '0') {
	add(sum, f[strlen(B)-1-i], sum);
      }
    }
    for (i = 119; i >= 0 && compare(f[i], sum) > 0; i--)
      ;
    ...
    while (i >= 0) {
      if (compare(f[i], sum) <= 0) {
	printf("1");
	subtract(sum, f[i], sum);
      } else {
	printf("0");
      }
      i--;
