			...
			BigInteger &r) const
{
  ...
  if (m >= 0) {
    ...
    for (int j = m; j >= 0; j--) {
      // D3: calculate q2
      Digit t = ((j+n < (int)r.mag.size()) ? r.mag[j+n] : 0) * BASE + 
	r.mag[j+n-1];
...
int main(void)
{
  BigInteger f[10001], pow2minus1[10001], t;
  int i, k, N, pow2computed;
  ...
  for (i = 2; i <= 10000; i++) {
    ...
    /* the rest */
    for (k = i-2; k >= 0 && pow2minus1[i-k] < f[i]; k--) {
      t = f[k];
      t *= 2;
      t += pow2minus1[i-k];
      if (t < f[i]) {
	f[i] = t;
      ...
      if (i-k+1 < 10001 && pow2computed < i-k+1) {
	pow2minus1[i-k+1] = pow2minus1[i-k];
	pow2minus1[i-k+1] *= 2;
	pow2minus1[i-k+1] += 1;
	pow2computed++;
      }
