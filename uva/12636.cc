...
BigInteger prod[200*199/2];
int n;



bool trial(const BigInteger &smallest, BigInteger best[])
{
  map<BigInteger,int> S;

  for (int i = 0; i < n*(n-1)/2; i++) {
    S[prod[i]]++;
  }

  //  cout << "trying smallest = " << smallest << endl;
  BigInteger A[200];
  A[0] = smallest;
  for (int i = 1; i < n; i++) {
    A[i] = S.begin()->first;
    ...
    if (A[i] % smallest != BigInteger(0)) return false;
    A[i] /= smallest;

    if (A[i] <= A[i-1]) return false;
    if (A[i] > BigInteger(1000000000)) return false;
    //    cout << "  A[" << i << "] = " << A[i] << endl;
  
    for (int j = 1; j < i; j++) {
      BigInteger p = A[i]*A[j];
      if (S.find(p) == S.end() || S[p] == 0) {
	//	cout << "bad because of " << p << " = " << A[i] << "*" << A[j] << endl;
	return false;
      }
      if (--S[p] == 0) {
	S.erase(p);
  ...
  if (lexicographical_compare(A, A+n, best, best+n)) {
    copy(A, A+n, best);
  }
  return true;
}
...
int main()
{
  ...
  for (int i = 1; i <= T; i++) {
    ...
    sort(prod, prod+n*(n-1)/2);

    BigInteger best[200];
    fill(best, best+n, INT_MAX);
    
    // we know that a0 = sqrt((a0*a1)*(a0*a2)/(a1*a2)).  We don't know
    // which one it is...but all the numbers < a1*a2 must be a0 * aj
    // so there can be at most n-2 of them
    for (int i = 2; i < n*(n-1)/2; i++) {
      BigInteger num = prod[0] * prod[1];
      if (num % prod[i] != BigInteger(0)) continue;
      num /= prod[i];
      BigInteger smallest;
      if (!num.sqrt(smallest)) continue;
      if (smallest < BigInteger(2) || smallest > BigInteger(1000000000) ) continue;
      trial(smallest, best);
    }
