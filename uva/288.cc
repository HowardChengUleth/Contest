...
BigInteger eval(const string &input)
{
  for (int i = input.length()-1; i >= 0; i--) {
    if (input[i] == '+') {
      return eval(input.substr(0, i)) + 
	eval(input.substr(i+1, input.length() - (i+1)));
    } else if (input[i] == '-') {
      return eval(input.substr(0, i)) -
	eval(input.substr(i+1, input.length() - (i+1)));
    }
  ...
  for (string::size_type i = 0; i < input.length(); i++) {
    if (input[i] == '*' && i < input.length()-1 && input[i+1] == '*') {
      return power(eval(input.substr(0, i)), 
		   eval(input.substr(i+2, input.length() - (i+2))));
    }
  }
