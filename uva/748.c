...
static void read_num(char *str, BigNum a);

/* write BigNum to stdout */
static void write_num(char *str, BigNum a);
...
int main(void)
{
  ...
  while (scanf("%s %d", str_in, &n) == 2) {
    for (i = strlen(str_in)-1; i >= 0 && str_in[i] == '0'; i--) {
      str_in[i] = 0;
    }
    d = 0;
    for (i = strlen(str_in)-1; i >= 0 && str_in[i] != '.'; i--) {
      d++;
    }
    do {
      str_in[i] = str_in[i+1];
    ...
    read_num(str_in, R);
    power(R, n, res);
    write_num(str_out, res);
    if (d == 0) {
      printf("%s.\n", str_out);
    } else if (d*n >= strlen(str_out)) {
      putchar('.');
      for (i = 0; i < d*n-strlen(str_out); i++) {
	putchar('0');
      }
      printf("%s\n", str_out);
    } else {
      for (i = 0; i < strlen(str_out)-d*n; i++) {
	putchar(str_out[i]);
      }
      putchar('.');
      printf("%s\n", str_out+i);
