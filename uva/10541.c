...
void compute(int *a, int n, int k)
{
  ...
  if (n >= 0 && k == 0) {
    init[n][k] = 1;
    convert_from_int(F[n][k], 1);
    return;
  }

  if (k == 1) {
    convert_from_int(F[n][k], n - a[k] + 1);
    return;
  }

  for (i = 1; i <= n - a[k] - 1; i++) {
    compute(a, n-a[k]-i, k-1);
    add(F[n][k], F[n-a[k]-i][k-1], F[n][k]);
  }
}
...
int main(void)
{
  ...
  while (cases-- > 0) {
    ...
    assert(1 <= n && n <= 200 && 0 <= k && k <= (200+1)/2);
