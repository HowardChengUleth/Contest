...
double dist(const Vector &p1, const Vector &p2)
{
  Vector p(p1, p2);
  return p.norm();
}
...
// distance from p to the line from a to b
...
double dist_point_to_face(const Vector &p, const Vector &a,
...
void do_case()
{
  ...
  double best = dist(t1[0], t2[0]);

  // vertex-face distance
  for (int i1 = 0; i1 < 4; i1++) {
    for (int j1 = 0; j1 < 4; j1++) {
      best = min(best, dist_point_to_face(t1[i1], t2[j1], t2[(j1+1)%4],
					  t2[(j1+2)%4]));
      best = min(best, dist_point_to_face(t2[i1], t1[j1], t1[(j1+1)%4],
					  t1[(j1+2)%4]));
    }
  }
