...
bool apply_op(stack<long long> &operands, Token token)
{
  long long a, b;
...
long long evaluate(string expr, bool &error, bool minimum)
{
  stack<Token> s;
...
int main(void)
{
  ...
  while (N-- > 0) {
    ...
    bool error;
    getline(cin, expr);
    long long minimum = evaluate(expr, error, true);
    long long maximum = evaluate(expr, error, false);
    cout << "The maximum and minimum are " << maximum << " and "
	 << minimum << "." << endl;
  }
  return 0;
}
