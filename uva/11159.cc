//
// Idea: form a bipartite graph with two sets A and B, and an edge
// a->b if a divides b.  We are looking for the size of the minimum
// vertex cover in the bipartite graph.  By Konig's theorem, this is
// the same as the size of the maximum matching.
//
...
int main(void)
{
  ...
  for (int t = 1; t <= T; t++) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < m; j++) {
	bipgraph[i][j] = B[j] == 0 || (A[i] != 0 && B[j] % A[i] == 0);
      }
    }
