...
int main(void)
{
  ...
  // person i has nodes 2*i, 2*i+1
  //
  // 2*i -> keep i on same team
  // 2*i+1 -> switch i to other team
  for (int i = 0; i < N; i++) {
    // can't do both
  ...
  for (int i = 0; i < M; i++) {
    ...
    // rivals must both switch or both stay put (or one of them removed)
    int u1 = 2*u, u2 = 2*u+1;
    int v1 = 2*v, v2 = 2*v+1;
    G.add_edge(u1, v2, INF);
    G.add_edge(v1, u2, INF);
  }
  ...
  for (int i = 0; i < N; i++) {
    ...
    // team 0 = 2 weight
    //
    // so real cost is 2-weight
    G.add_edge(s, 2*i, team[i] == '0' ? 2 : 1);
    G.add_edge(2*i+1, t, team[i] == '0' ? 1 : 2);
  }

  // find max weight independent set
