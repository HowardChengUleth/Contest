...
bool solve(int cap)
{
  ...
  // s = 0, t = 1, RA = 2..m+1, day = m+2 .. m+n+1

  for (int i = 0; i < m; i++) {
    G.add_edge(2+i, 1, cap);
  }
  for (int i = 0; i < n; i++) {
    G.add_edge(0, m+2+i, 2);
  }
  for (int i = 0; i < m; i++) {
    for (auto d : canwork[i]) {
      G.add_edge(m+2+d-1, 2+i, 1);
    }
  }
  int f = network_flow(G, 0, 1);
  if (f != 2*n) return false;

  cout << cap << endl;
  for (int i = 0; i < n; i++) {
    cout << "Day " << i+1 << ":";
    for (auto e : G.nbr[m+2+i]) {
