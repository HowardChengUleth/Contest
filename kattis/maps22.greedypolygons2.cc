...
int main()
{
  ...
  Point poly[60], hull[60];
  for (int i = 0; i < n; i++) {
    cin >> poly[i].x >> poly[i].y;
  }

  const double pi = acos(-1.0);
  double circle = pi * (d*g) * (d*g);
  
  n = convex_hull(poly, n, hull);  // overkill but put things in order

  double ans = 0.0;
  ans += area_polygon(hull, n);
  for (int i = 0; i < n; i++) {
    Point p1 = hull[i], p2 = hull[(i+1)%n], p3 = hull[(i+2)%n];
