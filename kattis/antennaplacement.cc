...
// the idea is that if you colour the cells like a checkerboard, then
// each antenna covers exactly one black and one white square.  So we
// form a bipartite graph and add an edge between two cells if both
// are points of interest.  The edges in a matching would be able to
// cover two points of interest so we find the maximum matching.  Any
// other points would each have to be covered with an additional
// antenna.
...
void solve()
{
  ...
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
      ...
      if ((i+j) % 2 != 0) continue;
      if (i > 0 && grid[i-1][j] == '*') {
	bipgraph[(i*w+j)/2][((i-1)*w+j)/2] = 1;
      }
      if (i < h-1 && grid[i+1][j] == '*') {
	bipgraph[(i*w+j)/2][((i+1)*w+j)/2] = 1;
      }
      if (j > 0 && grid[i][j-1] == '*') {
	bipgraph[(i*w+j)/2][(i*w+j-1)/2] = 1;
      }
      if (j < w-1 && grid[i][j+1] == '*') {
	bipgraph[(i*w+j)/2][(i*w+j+1)/2] = 1;
      }
