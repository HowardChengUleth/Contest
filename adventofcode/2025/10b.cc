...
int process(string line)
{
  ...
  // x[i] = how many times we press button i
  // objective function is just sum of button presses
  ...
  for (int i = 0; i < num_lights; i++) {
    for (int j = 0; j < num_buttons; j++) {
      A[row][j] = (buttons[j][i] ? 1 : 0);
      A[row+1][j] = (buttons[j][i] ? -1 : 0);
    }
    A[row][num_buttons] = joltage[i];
    A[row+1][num_buttons] = -joltage[i];
    row += 2;
  }

  int obj_val;
  int res = int_prog(A, num_buttons, num_lights*2, &obj_val, x);
