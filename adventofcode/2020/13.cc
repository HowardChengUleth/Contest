...
ll cra(int n, ll m[], ll a[]) {
  ...
  for (int i = 1; i < n; i++) {
    r = gcd(v, m[i], p, q);  t = v;
    if ((a[i] - u) % r != 0) { cout << "bad" << endl; }   // no solution!
...
bool checkB(ll t, vector<ll> id)
{
  for (int i = 0; i < (int)id.size(); i++) {
    if (id[i] == -1) continue;
    if ((t+i) % id[i] != 0) return false;
  }
  return true;
}

void partB(string line)
{
  for (auto &c : line) {
    if (c == ',') {
      c = ' ';
